...
1. **Plan 是单一事实来源：** 所有工作必须在 `执行计划.md` 中追踪。
...
7. **语言与注释一致性：** 所有文档、**代码注释（除非受环境限制）**、Git Commit Message 和 Git Notes 必须使用简体中文。
...
1. **选择任务：** 按顺序从 `执行计划.md` 中选择下一个可用任务。

2. **标记为进行中：** 在开始工作之前，编辑 `执行计划.md` 并将任务从 `[ ]` 更改为 `[~]`。
...
10. **获取并记录任务 Commit SHA:**
    - **步骤 10.1: 更新 Plan:** 阅读 `执行计划.md`，找到完成任务的行，将其状态从 `[~]` 更新为 `[x]`，并追加 *刚刚完成的 commit* 的 commit hash 的前 7 个字符。
    - **步骤 10.2: 写入 Plan:** 将更新的内容写回 `执行计划.md`。

11. **提交 Plan 更新：**
    - **行动：** 暂存修改后的 `执行计划.md` 文件。
    - **行动：** 使用描述性消息提交此更改（例如，`杂项: 标记任务 '创建用户模型' 为完成`）。

### 阶段完成验证与检查点协议 (Phase Completion Verification and Checkpointing Protocol)

**触发器：** 此协议在完成一个任务且该任务同时也结束了 `执行计划.md` 中的一个 Phase 后立即执行。

1.  **宣布协议开始：** 通知用户该阶段已完成，验证和检查点协议已开始。

2.  **确保阶段更改的测试覆盖率：**
    -   **步骤 2.1: 确定阶段范围：** 要识别在此阶段更改的文件，你必须首先找到起点。阅读 `执行计划.md` 以找到 *上一个* 阶段检查点的 Git Commit SHA。如果不存在上一个检查点，则范围是自第一次提交以来的所有更改。
    ...
    -   **步骤 2.3: 验证并创建测试：** 对于列表中的每个文件：
        ...
        -   如果测试文件缺失，你 **必须** 创建一个。在编写测试之前，**首先分析存储库中的其他测试文件以确定正确的命名约定和测试风格。** 新测试 **必须** 验证此阶段任务 (`执行计划.md`) 中描述的功能。
...
4.  **提议详细、可操作的手动验证计划：**
    -   **CRITICAL:** 要生成计划，首先分析 `产品手册.md`, `设计规范.md`, 和 `执行计划.md` 以确定完成阶段的面向用户的目标。
...
8.  **获取并记录阶段 Checkpoint SHA:**
    ...
    -   **步骤 8.2: 更新 Plan:** 阅读 `执行计划.md`，找到完成阶段的标题，并以 `[checkpoint: <sha>]` 格式追加 commit hash 的前 7 个字符。
    -   **步骤 8.3: 写入 Plan:** 将更新的内容写回 `执行计划.md`。
