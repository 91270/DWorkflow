...
-   **产品 (Product)**：定义项目上下文（例如用户、产品目标、高级功能）。
-   **产品设计规范 (Product guidelines)**：定义标准（例如文案风格、品牌信息、视觉识别）。
-   **技术架构 (Tech architecture)**：配置技术偏好（例如语言、数据库、框架、系统架构、运维方案）。
-   **工作流 (Workflow)**：设置团队偏好（例如 TDD、提交策略）。使用 [工作流模板.md](templates/工作流模板.md) 作为可自定义的模板。
...
/DW:newTrack
# 或者带描述
/DW:newTrack "为设置页面添加深色模式切换功能"
```

### 3. Implement the Track (实现任务清单)

批准执行计划后，运行 `/DW:implement`。你的编码智能体随后将按照 `执行计划.md` 文件工作，在完成任务时将其勾选。

**更新的工件：**
-   `.Docs/任务清单.md` (状态更新)
-   `.Docs/任务详情/<track_id>/执行计划.md` (状态更新)
-   项目上下文文件 (完成后同步)
...
3.  随着进度更新计划中的状态。
4.  **验证进度 (Verify Progress)**：在每个阶段结束时引导你进行手动验证步骤，以确保一切按预期工作。

在实现过程中，你还可以：

-   **检查状态 (Check status)**：获取项目进度的概览。
    ```bash
    /DW:status
    ```
-   **回滚工作 (Revert work)**：如果需要，撤销功能或特定任务。
    ```bash
    /DW:revert
    ```
